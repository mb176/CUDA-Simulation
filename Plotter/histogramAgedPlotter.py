...
for picIdx in range(1,nPictures):
    ...
    tMax = params[3];
    nTimes = len(data)-1;
    taMax  = 2*10**4; # THIS HAS TO BE SET MANUALLY!!! (SEE FILE NAME)
    ...
    for axesIdx in range(1,nAxes+1):
        ...
        # boundry t: plot vertical line indicating how far a particle can go in a single jump
        time = tMax/(nTimes-1)*(tIdx-1);
        boundry = time**nu;
        ax.axvline(boundry,color='r',label = 'Step with duration t', linestyle = '--',lw = linewidth)

        # boundry ta: plot vertical line indicating how far a particle can go in a single jump of duration ta
        taBoundry = (taMax+time)**nu - (taMax+time)**(nu-eta) * taMax**eta;
        ax.axvline(taBoundry,color='y',label = 'Step with duration ta+t', linestyle = '-.',lw = linewidth)

        #Labels
        plt.xlabel('Displacement in a.u.',axes=ax)
        plt.ylabel('Relative number of particles',axes=ax)
        plt.title(r't = %.2e a.u.'%time,axes=ax, fontsize = fontsize)
        plt.legend()
